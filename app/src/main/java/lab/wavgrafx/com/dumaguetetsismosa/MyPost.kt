package lab.wavgrafx.com.dumaguetetsismosa
...
import kotlinx.android.synthetic.main.activity_messenger.messenger_recyclerView
import kotlinx.android.synthetic.main.activity_messenger.progressbar1
import kotlinx.android.synthetic.main.activity_my_post.*
import kotlinx.android.synthetic.main.messages.view.*
import lab.wavgrafx.com.dumaguetetsismosa.Model.MessengerModel
...
class MyPost : AppCompatActivity() {
    ...
    private var adapter: MyPost.MessengerFirestoreRecyclerAdapter? = null

    lateinit var alias: Any
    lateinit var userblocked: Any
    lateinit var userpenaltycount: Any
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val query = rootRef!!.collection("Messages")
            .whereEqualTo("Phone", "$currentuser")
    ...
    private inner class MessengerViewHolder internal constructor(private val view: View) : RecyclerView.ViewHolder(view) {
        internal fun setMessengerDetails(Phone: String, Alias: String, Message: String, DateTimeSent: String, ReactionCount: String, Blocked: String) {
            ...
            view.btn_reaction.setOnClickListener{
                val intent = Intent(this@MyPost, Reactions::class.java)
                ...
                intent.putExtra("userblocked", "$userblocked")
                intent.putExtra("userpenaltycount", "$userpenaltycount")
...
//--------------------------------------------------------------------------------------------------
        ...
        if (!message.isEmpty()) {
            try {
                ...
                db.collection("Messages").document(messagedatetime).set(items).addOnSuccessListener {
                        void: Void? -> Toast.makeText(this, "Tsismis posted", Toast.LENGTH_SHORT).show()
                    messenger_recyclerView.smoothScrollToPosition(0)
    ...
    fun GetAlias(){
        db = FirebaseFirestore.getInstance().document("Tsismosa/Users")
            ...
            if (task.isSuccessful) {
                if (task.result!!.isEmpty){
                    ...
                    if (document != null) {
                        ...
                        userblocked = document.getValue("Blocked")
                        userpenaltycount = document.getValue("PenaltyCount")
                    }else{
