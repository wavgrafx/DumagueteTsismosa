...
class Profile : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_SaveAlias.setOnClickListener {
            ...
            if (alias.isEmpty()){
                Toast.makeText(this, "Please create an Alias for your privacy.", Toast.LENGTH_LONG).show()
            }else{
                checkConnectivity()
            ...
            if (task.isSuccessful) {
                ...
                if (document != null) {
                    ...
                    if (txtphone != "$currentuser"){
                        Toast.makeText(this, "This Alias was already taken.", Toast.LENGTH_SHORT).show()
                    }else{
...
//                  No such alias
    ...
    fun SaveAlias(){
        ...
        db.collection("List").document("$alias").set(items).addOnSuccessListener { void: Void? ->
            Toast.makeText(this, "Alias has been saved.", Toast.LENGTH_LONG).show()
        ...
        query.get().addOnCompleteListener(OnCompleteListener<QuerySnapshot> { task ->
            if (task.isSuccessful) {
                if (task.result!!.isEmpty){
                    Toast.makeText(this, "Please create an Alias", Toast.LENGTH_LONG).show()
                }else{
                    val intent = Intent(this@Profile, Messenger::class.java)
                    startActivity(intent)
                    finish()
